...
public partial struct TankSpawnSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        for (int i = 0; i < config.TankCount; i++)
        {
            ...
            var color = new HDRPMaterialPropertyBaseColor { Value = RandomColor(ref random) };
            ...
            foreach (var entity in linkedEntities)
            {
                ...
                if (state.EntityManager.HasComponent<HDRPMaterialPropertyBaseColor>(entity.Value))
                {
